...
sudo docker pull ditas/IMAGE_NAME:staging

# Get the host IP
HOST_IP="$(ip route get 8.8.8.8 | awk '{print $NF; exit}')"

# Run the docker mapping the ports and passing the host IP via the environmental variable "DOCKER_HOST_IP"
sudo docker run -p HOST_PORT:CONTAINER_PORT -e DOCKER_HOST_IP=$HOST_IP --restart unless-stopped -d --name IMAGE_NAME ditas/IMAGE_NAME:staging
ENDSSH
